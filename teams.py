...
DEADLINE = datetime.datetime(2011, 8, 21, 0) # Aug 21, 00:00
...
# Now we give race/class points to clans.
  ...
  for player in query.get_saints(c, owner):
    banner.safe_award_banner(c, player, 'beogh', 20, temp=True)
...
def update_clan_scores(c):
  ...
  for banner_name, player in top_clan_player_banners:
    banner.safe_award_banner(c, player, banner_name, 100, temp=True)
  for player in query.get_harvesters(c):
    banner.safe_award_banner(c, player, 'yredelemnul', 10)
